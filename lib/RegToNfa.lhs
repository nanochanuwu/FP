...
regexToNfa re = fst $ regexToNfaHelper re 1
...
regexToNfaHelper (Concat re1 re2) n = ( NFA states alphabet delta begin final , next ) where
    ...
    final = f2
    next = n2
regexToNfaHelper (Star re) n = ( NFA states alphabet delta begin final , next ) where
    (NFA s a d b f, n' ) = regexToNfaHelper re n
    states = s `union` [n']
    alphabet = a
    delta (st,sy)
